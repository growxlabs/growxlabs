...
export function MasterServiceAgreementDocument({ snapshot, showInternal = false }: { snapshot: MsaSnapshot; showInternal?: boolean }) {
  ...
  return <article className="mx-auto max-w-5xl overflow-hidden border border-slate-200 bg-white text-slate-900 shadow-sm print:shadow-none">
    <header className="border-b-2 border-slate-900 px-7 py-8 md:px-12">
      ...
      <p className="mt-10 text-xs font-bold uppercase tracking-[.2em] text-[#1d5f8d]">Service Agreement</p>
      <h1 className="mt-3 font-serif text-4xl leading-tight">{snapshot.document.title}</h1>
      <p className="mt-3 text-sm text-slate-600">This agreement records the services, responsibilities, fees, and terms agreed between the parties.</p>
      <dl className="mt-7 grid gap-x-8 gap-y-5 border-t border-slate-200 pt-5 sm:grid-cols-2 md:grid-cols-3">
        ...
        <Metadata label="Proposal reference" content={`${snapshot.sourceReferences.proposalNumber} · Version ${snapshot.sourceReferences.proposalVersion}`} /><Metadata label="Service scope reference" content={snapshot.sourceReferences.scopeNumber} /><Metadata label="Approval reference" content={snapshot.sourceReferences.proposalApprovalNumber} />
        <Metadata label="Issue date" content={date(snapshot.document.issuedAt)} /><Metadata label="Effective date" content={date(snapshot.document.effectiveAt)} /><Metadata label="Status" content={showInternal ? snapshot.document.status.replaceAll("_", " ") : clientStatus} />
      </dl>
    </header>
    ...
    <div className="px-7 md:px-12">{snapshot.sections.map((item) => <section key={item.key} className="border-b border-slate-200 py-9"><div className="mb-5 flex gap-4"><span className="font-serif text-xl text-[#1d5f8d]">{String(item.number).padStart(2, "0")}</span><h2 className="font-serif text-2xl">{item.title}</h2></div><div className="space-y-4 md:pl-11">{item.body.map((paragraph, index) => <p key={index} className="whitespace-pre-wrap text-sm leading-7 text-slate-700">{paragraph}</p>)}{item.items?.length ? <ul className="space-y-3">{item.items.map((entry, index) => <li key={index} className="flex gap-3 text-sm leading-7 text-slate-700"><span className="font-semibold text-[#1d5f8d]">•</span><span>{entry}</span></li>)}</ul> : null}{item.fields?.length ? <dl className="divide-y divide-slate-100 border-y border-slate-200">{item.fields.map((field, index) => <div key={index} className="grid gap-2 py-3 sm:grid-cols-[minmax(0,1fr)_2fr]"><dt className="text-xs font-bold uppercase tracking-wider text-slate-500">{field.label}</dt><dd className={field.value ? "text-sm text-slate-800" : "text-sm italic text-amber-700"}>{field.value || (showInternal ? "Internal review required" : "To be confirmed")}</dd></div>)}</dl> : null}</div></section>)}</div>
    <section className="border-t-2 border-slate-900 px-7 py-8 md:px-12"><h2 className="font-serif text-2xl">Pricing summary</h2><dl className="mt-5 grid gap-4 sm:grid-cols-2 md:grid-cols-4"><Metadata label="Total project value" content={money(totals.grand_total, currency)} /><Metadata label="Amount before tax" content={money(totals.taxable_subtotal, currency)} /><Metadata label="Tax" content={money(totals.tax_amount, currency)} /><Metadata label="Currency" content={currency} /></dl></section>
