...
export function RecruitmentPipeline({ candidates, onUpdateStage, updatingCandidateId, onRefresh, showRejected }: RecruitmentPipelineProps) {
  ...
  const convert = async (offerOverride = false) => {
    ...
    setConversionError(""); setConversionBusy(true);
    try {
      const response = await fetch(`/api/admin/recruitment/applications/${converting.id}/convert-to-employee`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...conversion, managerEmployeeId: conversion.managerEmployeeId || null, roleId: conversion.roleId || null, offerOverride, offerOverrideReason: offerOverride ? offerOverrideReason : undefined }) });
      const body = await response.json().catch(() => null);
      if (!response.ok || !body?.success) { setOfferOverrideRequired(body?.error?.code === "offer_required"); return setConversionError(body?.error?.message || "We couldn't create this employee."); }
      setConversionResult(body.conversion); onRefresh?.();
    } catch { setConversionError("We couldn't reach the employee service. Please try again."); }
    finally { setConversionBusy(false); }
  };
  const resendActivation = async () => {
    if (!conversionResult?.employeeId || resendBusy) return;
    setResendBusy(true); setConversionError("");
    try {
      const response = await fetch(`/api/admin/employees/${conversionResult.employeeId}/activation/reissue`, { method: "POST" });
      const body = await response.json().catch(() => null);
      if (!response.ok || !body?.success) return setConversionError(body?.error?.message || "The activation email could not be reissued.");
      setConversionResult((result: any) => ({ ...result, activationEmail: body.activationEmail }));
    } catch { setConversionError("We couldn't reach the email service. Please try again."); }
  ...
  return (
    <>
                  ...
                  {stageCandidates.map((candidate: any) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between pt-1 border-t border-[var(--border-subtle)]">
                        ...
                        {!showRejected && stage === "OFFER" && <Link href={`/admin/offers?applicationId=${encodeURIComponent(candidate.id)}&title=${encodeURIComponent(candidate.job_title || "")}`} className="text-[8px] font-bold uppercase text-emerald-700 hover:underline">Prepare Offer</Link>}
                        {!showRejected && stage === "HIRED" && candidate.employee_id && <Link href={`/admin/offers?applicationId=${encodeURIComponent(candidate.id)}&title=${encodeURIComponent(candidate.job_title || "")}&backfill=1`} className="text-[8px] font-bold uppercase text-violet-700 hover:underline">Create Missing Offer</Link>}
                        {!showRejected && stage === "HIRED" && <button type="button" onClick={() => { setConverting(candidate); setConversionResult(null); setConversionError(""); setOfferOverrideRequired(false); setOfferOverrideReason(""); }} className="text-[8px] font-bold uppercase text-emerald-600 hover:underline">Convert to Employee</button>}
      ...
      {converting && <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/60 p-4"><div className="w-full max-w-lg rounded-xl bg-[var(--card)] p-5 shadow-2xl"><div className="flex justify-between"><div><h2 className="text-base font-bold">{conversionResult ? "Employee conversion" : "Convert to Employee"}</h2><p className="mt-1 text-xs text-[var(--text-secondary)]">{converting.full_name} · Workspace remains pending until activation.</p></div><button onClick={() => setConverting(null)}><X className="h-4 w-4" /></button></div>{conversionResult ? <div className="mt-5 space-y-3"><div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-xs text-emerald-900"><p className="font-bold">{conversionResult.existing ? "Employee already created" : "Employee created"}</p><p className="mt-1">{converting.full_name} has been converted to an employee.</p><dl className="mt-4 grid grid-cols-2 gap-y-2"><dt>Employee code</dt><dd className="font-semibold">{conversion.employeeCode}</dd><dt>Workspace</dt><dd className="font-semibold">{conversionResult.workspaceStatus === "active" ? "Active" : "Pending activation"}</dd><dt>Activation email</dt><dd className="font-semibold">{conversionResult.activationEmail?.sent ? "Sent" : "Failed"}</dd></dl></div>{!conversionResult.activationEmail?.sent && <button onClick={() => void resendActivation()} disabled={resendBusy} className="w-full rounded-lg bg-amber-600 px-4 py-2 text-xs font-bold text-white disabled:opacity-50">{resendBusy ? "Sending…" : "Resend activation email"}</button>}{conversionError&&<p className="text-xs text-red-600">{conversionError}</p>}<div className="flex justify-end"><button onClick={() => setConverting(null)} className="rounded-lg border border-[var(--border-subtle)] px-4 py-2 text-xs font-bold">Close</button></div></div> : <><div className="mt-5 grid grid-cols-2 gap-3">{[{key:"employeeCode",label:"Employee code"},{key:"departmentId",label:"Department ID"},{key:"designationId",label:"Designation ID"},{key:"managerEmployeeId",label:"Manager ID (optional)"},{key:"joiningDate",label:"Joining date",type:"date"},{key:"roleId",label:"Role ID (optional; defaults to BDE)"}].map(field=><label key={field.key} className="text-[10px] font-bold uppercase">{field.label}<input type={field.type||"text"} value={(conversion as any)[field.key]} onChange={e=>setConversion({...conversion,[field.key]:e.target.value})} className="mt-1 w-full rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-1)] p-2 text-xs normal-case" /></label>)}</div>{conversionError&&<p className="mt-3 text-xs text-red-600">{conversionError}</p>}{offerOverrideRequired&&<div className="mt-3 rounded-lg border border-amber-300 bg-amber-50 p-3"><label className="text-[10px] font-bold uppercase text-amber-900">Admin override reason<textarea value={offerOverrideReason} onChange={e=>setOfferOverrideReason(e.target.value)} className="mt-1 min-h-20 w-full rounded border border-amber-300 bg-white p-2 text-xs normal-case"/></label><button onClick={()=>void convert(true)} disabled={conversionBusy||offerOverrideReason.trim().length<3} className="mt-2 rounded bg-amber-700 px-3 py-2 text-xs font-bold text-white disabled:opacity-50">Convert Without Offer</button></div>}<div className="mt-5 flex justify-end"><button onClick={()=>void convert()} disabled={conversionBusy||!conversion.employeeCode||!conversion.departmentId||!conversion.designationId||!conversion.joiningDate} className="rounded-lg bg-emerald-600 px-4 py-2 text-xs font-bold text-white disabled:opacity-50">{conversionBusy ? "Converting…" : "Convert to Employee"}</button></div></>}</div></div>}
    </>
  );
}
