...
export async function PATCH(request:Request,{params}:{params:Promise<{offerId:string}>}){
  ...
  const stored=await supabaseAdmin.storage.from('hrms-documents').upload(objectKey,pdf.bytes,{contentType:'application/pdf',upsert:false});if(stored.error)return Response.json({error:"Offer PDF could not be stored",detail:stored.error.message},{status:500});
