...
export function validateProductionConfiguration(
  ...
  const environment = source.APP_ENV ?? (source.VERCEL_ENV === "production" ? "production" : source.VERCEL_ENV === "preview" ? "preview" : "development");
  if (environment === "development" || environment === "test") {
    return { ready: true, environment, missing: [], invalid: [] };
  }
  const parsed = ProductionConfigSchema.safeParse({ ...source, APP_ENV: environment });
