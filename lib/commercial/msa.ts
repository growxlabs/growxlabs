...
function buildSections(input: { accepted: JsonRecord; sow: JsonRecord; client: JsonRecord; growx: JsonRecord; proposalNumber: string; proposalVersion: number; scopeNumber: string; approvalNumber: string; agreementNumber: string; version: number; issuedAt: string; effectiveAt: string | null; paymentSchedule: unknown[]; pricing: unknown[]; totals: JsonRecord; signatories: MsaSnapshot["signatories"] }): MsaSection[] {
  ...
  const paymentItems = asArray(input.paymentSchedule).map((entry, index) => {
    ...
    return `${text(record.name) && !/^₹?\s*[\d,]+(?:\.\d+)?$/.test(text(record.name)) ? text(record.name) : `Payment stage ${index + 1}`} — ${percentage}% (${money(amount, currency)}). ${text(record.trigger)}`.trim();
  });
  ...
  return [
    section(1, "parties", "Parties and Agreement Information", [
      ...
      "This agreement becomes effective once signed by both parties and any required details are completed.",
    ], [], [
      ...
      { label: "Proposal reference", value: `${input.proposalNumber} · Version ${input.proposalVersion}` },
      { label: "Service scope reference", value: input.scopeNumber },
      { label: "Approval reference", value: input.approvalNumber },
    ]),
    section(2, "engagement", "Engagement and Purpose", [text(acceptedContent.executiveSummary) || "The purpose of the engagement is described in the service proposal and service scope referenced in this agreement." ]),
    section(3, "scope", "Scope of Services", [
      `Services are limited to the Service Scope ${input.scopeNumber} and the Commercial Proposal ${input.proposalNumber}, Version ${input.proposalVersion}.`,
      "The Service Scope defines the work included in this engagement. Additional requirements are included only when both parties approve a written Change Request.",
    ], [...items(sowContent.included), ...items(acceptedContent.scope)]),
    section(4, "deliverables", "Deliverables", ["The deliverables below form part of the agreed services. Any additional deliverable requires a written Change Request."], items(sowContent.deliverables).length ? items(sowContent.deliverables) : items(acceptedContent.deliverables)),
    section(5, "timeline", "Project Timeline and Milestones", ["Work will follow the sequence described in the Service Scope. Dates may depend on required inputs, approvals, access, and third-party dependencies."], items(sowContent.milestones).length ? items(sowContent.milestones) : items(acceptedContent.milestones || acceptedContent.timeline)),
    section(6, "commercial", "Commercial Terms", ["The pricing and charges below reflect the Commercial Proposal referenced in this agreement."], asArray(input.pricing).map((entry, index) => { const line = asRecord(entry); return `${text(line.description) || `Service line ${index + 1}`} — quantity ${text(line.quantity) || "-"}, unit ${text(line.unit) || "-"}, unit price ${money(line.unit_price ?? line.unitPrice, currency)}, discount ${money(line.discount, currency)}, line amount ${money(line.subtotal, currency)}.`; }), [
      { label: "Currency", value: currency },
      { label: "Total project value", value: money(input.totals.grand_total, currency) },
      { label: "Amount before tax", value: money(input.totals.taxable_subtotal, currency) },
      { label: "Tax amount", value: money(input.totals.tax_amount, currency) },
    ]),
    section(7, "payment", "Payment Terms", ["Payments are due according to the schedule below. Each amount is based on the total project value and the percentage shown."], paymentItems),
    section(8, "taxes", "Taxes", [
      `The applicable tax treatment is ${text(input.totals.tax_type) || "the agreed tax treatment"} at ${text(input.totals.tax_rate) || "0"}%. Taxes will be handled in accordance with applicable law and the agreed commercial terms.`,
    ]),
    section(9, "clientResponsibilities", "Client Responsibilities", ["Client will provide the information, content, assets, approvals, credentials, access, and authorised points of contact reasonably required for the agreed services."], items(sowContent.assumptions).concat(items(sowContent.dependencies))),
    section(10, "growxlabsResponsibilities", "GrowxLabs Responsibilities", ["GrowxLabs will perform the agreed services with reasonable care, coordinate the agreed milestones, communicate material dependencies, and deliver the agreed deliverables for review."], items(sowContent.deliverables)),
    ...
    section(12, "changeRequests", "Change Requests and Additional Work", ["Only a written Change Request approved by both parties becomes part of the engagement. It must record each of the following before work begins on the requested change."], ["Requested change", "Reason", "Scope impact", "Timeline impact", "Commercial impact", "GrowxLabs approval", "Client approval"]),
    section(13, "thirdParty", "Third Party Services and Integrations", ["Agreed integrations may depend on Google services, Meta services, WhatsApp services, analytics services, hosting, domains, Tally, external APIs, cloud infrastructure, AI services, and other agreed integrations. GrowxLabs is responsible for its agreed implementation work, not for third-party outages, pricing changes, access restrictions, policy changes, service discontinuation, or performance outside GrowxLabs' control."], items(sowContent.dependencies).filter((value) => /third.party|domain|hosting|google|meta|whatsapp|api|tally|analytic|cloud|AI/i.test(value))),
    ...
    section(15, "intellectualProperty", "Intellectual Property", ["Ownership and licensing for custom deliverables will follow the treatment recorded in this agreement."], ["Client Materials — Client retains ownership of its existing brand assets, product information, content, trademarks, data, and other materials supplied to GrowxLabs.", "GrowxLabs Background Technology — GrowxLabs retains ownership of pre-existing technology, reusable software, internal tools, frameworks, components, libraries, development utilities, automation infrastructure, AI infrastructure, methods, and general technical know-how.", "Third Party Components — Third-party and open-source components remain subject to their respective licences and terms.", "Custom Deliverables — Ownership or licensing for custom deliverables created specifically for the Client must be agreed by the parties before signing."]),
    section(16, "clientMaterials", "Client Materials and Data", ["Client is responsible for the accuracy, lawfulness, permissions, and appropriate access rights for Client Materials and data supplied to GrowxLabs. GrowxLabs will use them for this engagement and will follow the agreed handling instructions available at the time of delivery."]),
    section(17, "confidentiality", "Confidentiality", ["Each party will protect non-public business, technical, commercial, and other confidential information received from the other party and use it only for this engagement, subject to applicable law and agreed disclosures. Any additional confidentiality terms required for this engagement will be included in the signed agreement."]),
    section(18, "acceptance", "Testing and Acceptance", ["Testing and acceptance will follow the acceptance criteria for the agreed services. New functionality or material modification requested during review that is outside the agreed scope follows the Change Request process."], items(sowContent.acceptanceCriteria).length ? items(sowContent.acceptanceCriteria) : items(acceptedContent.terms).filter((value) => /accept|test|review|defect/i.test(value))),
    section(19, "deployment", "Production Deployment and Handover", ["Production deployment, agreed configuration, testing, administrator access, documentation, and handover are limited to the agreed deliverables and available third-party access. Any additional deployment or operational requirement requires a written Change Request."]),
    section(20, "support", "Support and Maintenance", ["Ongoing support and maintenance are not included unless separately agreed in writing. Any support services, service levels, response times, or fees will be described in a separate agreement or Change Request." ]),
    section(21, "warranties", "Warranties and Disclaimers", ["GrowxLabs will perform the agreed services with reasonable care. Except for commitments expressly stated in this signed agreement, no additional warranty, performance guarantee, availability commitment, or outcome guarantee is created. Third-party services remain subject to their own terms and availability."]),
    section(22, "liability", "Limitation of Liability", ["Any liability cap, exclusions, carve-outs, and treatment of indirect or consequential loss must be stated expressly in the signed agreement."]),
    section(23, "termination", "Termination", ["Termination rights, notice periods, cure mechanics, and termination for convenience or cause will be those stated in the signed agreement."]),
    section(24, "terminationConsequences", "Consequences of Termination", ["On termination, the parties will confirm treatment of completed work, unpaid fees, Client Materials, access, confidential information, and any third-party commitments in the signed agreement. Amounts due for work actually performed will be handled according to the agreed commercial terms."]),
    section(25, "forceMajeure", "Force Majeure", ["The parties may agree on the definition, notice duties, mitigation steps, and consequences for events beyond reasonable control, including infrastructure or service interruptions."]),
    section(26, "governingLaw", "Governing Law and Dispute Resolution", ["The signed agreement will state the governing law and dispute-resolution procedure, including any applicable courts, arbitration, venue, escalation, and related process."]),
    section(27, "notices", "Notices", ["Formal notices should use the legal names, addresses, email addresses, and delivery method stated in the signed agreement."]),
    section(28, "entireAgreement", "Entire Agreement and Amendments", [`The signed agreement, the Commercial Proposal ${input.proposalNumber} Version ${input.proposalVersion}, and the Service Scope ${input.scopeNumber} together describe the agreed engagement. Any amendment or scope expansion must be recorded in writing and approved by both parties through the applicable process.`]),
    section(29, "electronicExecution", "Electronic Execution", ["The parties may sign electronically. Each electronic signature is recorded with the authorised signatory's consent and signing date." ]),
    section(30, "signatories", "Authorised Signatories", ["Each signature must include the signatory's full legal name, role, company, email, signature, consent to electronic execution, and signing date."], [], input.signatories.map((signatory) => ({
      label: signatory.party === "client" ? `For ${clientName}` : "For GrowxLabs",
...
async function sendAgreement(agreement: JsonRecord, version: JsonRecord, actorId: string) {
  ...
  const body = GrowXLabsEmailLayout({ context: "SERVICE AGREEMENT", reference: agreement.agreement_number, content: `<p>Hello ${escapeGrowXLabsEmailHtml(displayName)},</p><p>Your GrowxLabs Service Agreement <strong>${escapeGrowXLabsEmailHtml(agreement.agreement_number)}</strong> is ready for review in the Client Portal.</p>${EmailAction("Review Agreement", `${process.env.NEXT_PUBLIC_APP_URL || "https://growxlabs.tech"}${reviewUrl}`)}<p>Please review the agreement and use the signing action in the Client Portal when you are ready.</p>` });
...
export async function getClientAgreement(reference: string, clientId: string, userId: string, track = true) {
  ...
  if (!version.client_visible) return fail(409, "VERSION_NOT_CLIENT_VISIBLE", "This agreement version is not available in the Client Portal.");
...
export async function getAgreementPdfForClient(reference: string, clientId: string, userId: string) {
  ...
  return { ...data, pdf: await renderMasterServiceAgreementPdf(data.snapshot, "client") };
}
