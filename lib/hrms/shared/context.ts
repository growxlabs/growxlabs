...
const employeeCapabilityAliases:Record<string,string[]>={
  "attendance.view_self":["employee.attendance.read_own"],"attendance.clock":["employee.attendance.use"],
  "leave.view_self":["employee.leave.read_own"],"leave.request":["employee.leave.request"],
};
export function requirePermission(context:HRMSContext,...allowed:string[]){if(!allowed.some(permission=>context.permissions.has(permission)||(employeeCapabilityAliases[permission]||[]).some(alias=>context.permissions.has(alias))))throw new HRMSError(403,"forbidden",`Required permission: ${allowed.join(" or ")}`)}
