...
export type AssessmentInformationRequest = { id: string; message: string; requestedQuestionKeys: string[]; requestedSectionKeys: string[]; status: "draft" | "sent" | "in_progress" | "submitted" | "under_review" | "partially_accepted" | "changes_required" | "resolved" | "cancelled" | "open" | "answered"; dueAt: string | null; createdAt: string };
export type ClientAssessment = { id: string; assessmentNumber: string; templateId: string; templateVersion: number; clientId: string; companyId: string | null; leadId: string | null; dealId: string | null; status: ClientAssessmentStatus; currentSection: number; completedSections: string[]; completionPercentage: number; template: AssessmentTemplate; answers: Record<string, AssessmentAnswerValue>; files: AssessmentFile[]; informationRequests: AssessmentInformationRequest[]; startedAt: string | null; submittedAt: string | null; updatedAt: string };
