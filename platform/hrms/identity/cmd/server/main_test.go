...
func TestActorFromRejectsPartialIdentity(t *testing.T) {
	...
	if _, err := actorFrom(request); err == nil {
		t.Fatal("expected missing organisation and request ID error")
	}
}
...
func TestActorFromDoesNotInferAdminRole(t *testing.T) {
	...
	if ac.Permissions["organisation.manage"] {
		t.Fatal("legacy ADMIN role must not grant database permission")
	}
}
