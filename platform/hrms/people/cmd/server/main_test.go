...
func TestActorFromRequiresTenantContext(t *testing.T) {
	...
	if _,err:=actorFrom(request);err==nil{t.Fatal("expected missing context error")}
}
...
func TestActorFromParsesPermissions(t *testing.T) {
	...
	if !ac.Permissions["employee.view"]||!ac.Permissions["manager.view_team"]{t.Fatal("permissions were not parsed")}
}

func TestPositiveIntBounds(t *testing.T) {
	if got:=positiveInt("-1",20,1,100);got!=20{t.Fatalf("expected fallback, got %d",got)}
	if got:=positiveInt("500",20,1,100);got!=100{t.Fatalf("expected cap, got %d",got)}
}
